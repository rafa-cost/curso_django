...
@pytest.fixture     # para executar o login , eu preciso que o usuario esteja salvo no meu banco, e a classe djando_user_model faz essa conferência
...
def resp_post(client, usuario):    #vamos receber esses dados do dicionario antes de fazer a chamada post
...
def resp_home(client, db):                     #se conectando com o banco de dados
    return client.get(reverse('base:home'))    # o resp_home direcionando para home.html
def test_botao_entrar_disponivel(resp_home):     #teste botão entrar, estara disponivel assim que o usuário acessar o site, depois que ele se logar o botão desaparece
    assert_contains(resp_home, 'Entrar')

def test_link_de_login_disponivel(resp_home):    #o teste link de login onde coloca as informações para se logar
